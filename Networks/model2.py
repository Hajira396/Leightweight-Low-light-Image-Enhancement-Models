...
# Residual Group containing CAB
class Res(nn.Module):
    def __init__(self, n_feat, n_resblocks=2, reduction=8, dilation=1):
        super(Res, self).__init__()
        self.resblocks = nn.ModuleList([CAB(n_feat, reduction, dilation=dilation) for _ in range(n_resblocks)])
...
class Model2(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=32, n_resgroups=2, n_resblocks=2, reduction=8, dilation=1):
        ...
        self.resgroups = nn.ModuleList([Res(n_feat, n_resblocks, reduction, dilation) for _ in range(n_resgroups)])
        self.rdb = RDB(n_feat)
