...
class Res(nn.Module):
    def __init__(seRelf, n_feat, n_resblocks=2, reduction=8):
        super(Res, self).__init__()
        self.resblocks = nn.ModuleList([CAB(n_feat, reduction) for _ in range(n_resblocks)])
...
class Model1(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=16, n_resgroups=2, n_resblocks=2, reduction=8):
        ...
        self.resgroups = nn.ModuleList([Res(n_feat, n_resblocks, reduction) for _ in range(n_resgroups)])
